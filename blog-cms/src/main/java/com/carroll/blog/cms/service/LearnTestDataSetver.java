...
class LearnTestDataSetver implements Runnable {

    private LearnTestMapper learnTestMapper;

    public LearnTestDataSetver(LearnTestMapper learnTestMapper) {
        this.learnTestMapper = learnTestMapper;
    }

    void contextLoads() {
        LearnTestDataSetver l = new LearnTestDataSetver(learnTestMapper);
        new Thread(l, "1").start();
        new Thread(l, "2").start();
        new Thread(l, "3").start();
        new Thread(l, "4").start();
    ...
    public void run() {
        ...
        for (int i = 0; i < 2000000; i++) {
            learnTestMapper.insert(add());
        }
        System.out.println(Thread.currentThread().getName() + "打印完了！！");

    }


    private static LearnTest add() {
        ...
        for (int j = 0; j < 1; j++) {
            rs.append(source.charAt(r.nextInt(3)));
        }
        learnTest.setName(getRandomJianHan(Integer.parseInt(rs.toString())));
        learnTest.setSex(r.nextBoolean());
        learnTest.setCreateDate(new Date());
        learnTest.setPhone(getPhoneNum());
        learnTest.setAddress(getRoad());
        return learnTest;
    }
